...
public class Bag extends javax.swing.JFrame {
    ...
    private void movePokemonToBag( DefaultListModel model, DefaultListModel toModel) {
        
          int index = computerList.getSelectedIndex();
          
          if(index != -1) {
          toModel.addElement(model.get(index));
          model.remove(index);
          }
    ...
    private void movePokemonsToBag(DefaultListModel model, DefaultListModel toModel) {
        int[] indices = computerList.getSelectedIndices();
        
        for(int i = 0; i < indices.length; i++) {
            if(indices.length > 0) {
                toModel.addElement(model.get(i));
                model.remove(i);
            }
        }
    }
    
        private void movePokemonsToComputer(DefaultListModel model, DefaultListModel toModel) {
    ...
    private void moveBackPackButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_moveBackPackButtonActionPerformed
       int[] indices = computerList.getSelectedIndices();
        ...
        if(indices.length == 1) {
            movePokemonToBag(computerModel, backPackModel);
        }
        
        else  {
            movePokemonsToBag(computerModel, backPackModel);
        }
    ...
    private void moveComputerButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_moveComputerButtonActionPerformed
        ...
        if(indices.length == 1) {
            movePokemonToComputer(backPackModel, computerModel);
        }
        
        else  {
            movePokemonsToComputer(backPackModel, computerModel);
        }
